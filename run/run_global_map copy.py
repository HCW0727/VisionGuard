...
def overlap(org_img,overlap_img,moved_x,moved_z,current_angle):
    ...
    if changed_x < 0:
        ...
        #As org_width increases, Same Position becomes lower, so we need to increase the same value to current pose
        current_x += left_pad
    ...
    if current_angle != 0:
        ...
        # triangle_roi_mask,triangle_roi_mask_inv
        masked_original = cv2.bitwise_and(img_with_border[changed_z-pre_height:changed_z,
                                        changed_x:changed_x+pre_width],triangle_roi_mask_inv)
        overlap_img = cv2.bitwise_or(overlap_img_no_triangle,masked_original)
...
if __name__ == '__main__':
    start_time = time.time()
    org_img = cv2.imread('./output/image/BEV/000100.png', cv2.IMREAD_GRAYSCALE)

    # org_img = cv2.imread('./saved_image.png',cv2.IMREAD_GRAYSCALE)
    overlap_img = cv2.imread('./output/image/BEV/000101.png',cv2.IMREAD_GRAYSCALE)

    # result = overlap(org_img,overlap_img,0,0,-30)
