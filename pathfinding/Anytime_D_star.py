...
class ADStar:
    def __init__(self, img_map, s_start, s_goal, eps, heuristic_type):
        self.img_map = img_map
        self.s_start, self.s_goal = s_start, s_goal
        self.heuristic_type = heuristic_type

        self.Env = Env(self.img_map) 
        ...
        #CLOSED : visited node, INCONS : visited node(not in optimal path)
    ...
    def on_press(self, event):
        ...
        if x < 0 or x > self.x - 1 or y < 0 or y > self.y - 1:
            ...
            if self.title == "Anytime D*: Small changes":
                ...
                #if clicked block is obs, remove it, if not, add it to obstacle list.
...
def main():
    img_map = cv2.imread('global_map.png',cv2.IMREAD_GRAYSCALE)
    ...
    # img_map = cv2.imread('pathfinding/global1.png',cv2.IMREAD_GRAYSCALE)
    

    dstar = ADStar(img_map, s_start, s_goal, 2.5, "euclidean")
